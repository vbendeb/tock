...
impl ProcessPrinter for ProcessPrinterText {
    fn print(
        ...
        let mut iteration = match context {
            ProcessPrinterContext::PrintAll => 0,
            ProcessPrinterContext::PrintSome(i) => i.0,
            ProcessPrinterContext::PrintSomeNoMpu(i) => i.0,
        };
        ...
        loop {
            let keep_going = match iteration {
                ...
                3 => {
                    ...
                    // SRAM sizes
                    let sram_grant_size =
                        process_struct_memory_location - addresses.sram_grant_start;
                ...
                11 => {
                    let print_mpu = match context {
                        ProcessPrinterContext::PrintAll => true,
                        ProcessPrinterContext::PrintSome(_) => true,
                        _ => false,
                    };
                    if print_mpu {
                        process.print_mpu_config(writer);
                    }
                    true
                }
            ...
            if keep_going {
                if context == ProcessPrinterContext::PrintAll {
                    iteration += 1;
                } else {
                    return Some(Iteration(iteration + 1));
                }
            } else {
                return None;
            }
